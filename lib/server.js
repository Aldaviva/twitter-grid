	...
	if(!shutdownPromise){
		shutdownPromise = startedPromise
			.finally(function(){
				...
				clearInterval(fetchTweetsInterval);
				return Q()
...
var tweetService = require('./service/tweetService');
startedPromise.then(function(){
	tweetService.fetchAndStoreMoreTweets();
	fetchTweetsInterval = setInterval(tweetService.fetchAndStoreMoreTweets, config.twitter.pollingInterval);
});